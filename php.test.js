...
test('Determine whether a variable is empty', () => {
	expect(empty(0)).toBe('Sunday');
});
...
test('Convert string to first letter uppercase', () => {
	expect(ucfirst('THE FIRST LETTER UPPERCASE')).toBe('The first letter uppercase');
});
...
test('Get week names', () => {
	expect(week_name(0)).toBe('Sunday');
	expect(week_name(1)).toBe('Monday');
	expect(week_name(2)).toBe('Tuesday');
	expect(week_name(3)).toBe('Wednesday');
	expect(week_name(4)).toBe('Thursday');
	expect(week_name(5)).toBe('Friday');
	expect(week_name(6)).toBe('Saturday');
});
